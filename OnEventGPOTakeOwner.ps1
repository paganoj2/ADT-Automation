...
This runs on event 5137 Directory Services Changes, specifically GPO creation. Once a GPO creation event is detected, this script will pull
the information of the newly created GPO and run a take ownership task to make "Domain Admins" the owner, instead of the user who created it.
This will also add two groups, a Forest GPO admins group and a Domain GPO admins group to the ACL.
...
#Get GPO Information from Event Log
$StartTime = ((Get-Date).AddHours(-4))
$eventLog = Get-WinEvent -FilterHashtable @{ProviderName = 'Microsoft-Windows-Security-Auditing'; LogName = 'Security'; ID = 5137; StartTime = $StartTime}
...
foreach ($event in $eventLog) {
    ...
    $name = $eventData.Name
    $value = $eventData.InnerText
    $GPOData[$eventData] = $value
    $ObjectDN = $eventData | Where-Object { $_.Name -eq 'ObjectDN' } | Select-Object -ExpandProperty InnerText
    $GPOId = [regex]::Match($ObjectDN, '(?<={)[^{}]+(?=})').Value
    #}
    if(Get-GPO -Guid $GPOId -Server $DomainController -ErrorAction SilentlyContinue){
    $GPOOwner = Get-GpoOwner -DomainController $DomainController -GPOId $GPOId
    if ($GPOOwner -notlike "$ForestNetBiosName\$NewOwner"){
    Change-GpoOwner -NewOwnerSID $NewOwnerSID -DomainController $DomainController -GPOId $GPOId
    Set-GPPermissions -Guid $GPOId -PermissionLevel GpoEditDeleteModifySecurity -TargetName "$DomainNetBiosName\$GPOAdminsGroup" -TargetType Group -DomainName $Domain -Server $DomainController
    Set-GPPermissions -Guid $GPOId -PermissionLevel GpoEdit -TargetName "$DomainNetBiosName\$GPOEditorsGroup" -TargetType Group -DomainName $Domain -Server $DomainController
    Set-GPPermissions -Guid $GPOId -PermissionLevel GpoEditDeleteModifySecurity -TargetName "$ForestNetBiosName\$ForestGPOAdminsGroup" -TargetType Group -DomainName $Domain -Server $DomainController
    Set-GPPermissions -Guid $GPOId -PermissionLevel GpoEdit -TargetName "$ForestNetBiosName\$ForestGPOEditorsGroup" -TargetType Group -DomainName $Domain -Server $DomainController
    }
    }
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUk368m7NY96zZQx60sq8zwc6J
# kWiggg9rMIIHsTCCBZmgAwIBAgITNQAAACFv8bfVN7E2EQAAAAAAITANBgkqhkiG
...
# IwYJKoZIhvcNAQkEMRYEFPYT7qAk0M8eM23kItAQTKMIGMuVMA0GCSqGSIb3DQEB
# AQUABIIBAGkgutfbkQUvaKNRMcw9QFpFOlRyya2pKF/cTaGgji3ChnY2wpDvJMkb
# Oyzt6MI0PpNP9Wp+X9l2qwc19gUxdg1Gm86UkJFeoTJeq8+wd/6Hps03J81jeIMY
# p4dnFTdr10g2LzyYeLlOFypLFbHUXMdh0+WS6VJOjugdRQSX7bz1s8uyv66XnD3P
# /dVz7TW9fz6lCYWgPPULp6nYtA7oxhgn7q7/dAuR34FBaKYRG3hdA0zndQp7j4At
# WeYQWHvlKjfISHqhGsZ48AaPubtBbHb9j69hlVpgo0t1GQoG2F/+Cs6LZ/87+Wcr
# RddRC7Gmr7mVYykrFu6if/ZmhHpFI5Q=
