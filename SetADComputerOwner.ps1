...
This runs because of an update from Microsoft. Microsoft has made active directory computer accounts only joinable by the owner of the computer object.
This breaks SCCM functionality because sysadmins create computer objects, and then when SCCM runs to do a computer re-image, an SCCM service account
is responsible for performing the domain join. This domain join cannot occur unless the owner of the computer object is the SCCM service account.



.VERSION CONTROL

6/19/2024 - I was born.

6/19/2024 - Set as a scheduled task to run on all servers in the forest.
...
#>
$Domain = "RES"
$DomJoinAccount = "SVC.SCCM002"

$AllComputers = Get-ADComputer -Filter * -Properties nTSecurityDescriptor |sort name
$objuser = New-Object System.Security.Principal.NTAccount($Domain,$DomJoinAccount)

 foreach ($c in $AllComputers) {
    
    $Owner = $c.nTSecurityDescriptor.Owner
    $dn = $c.distinguishedname
    if ($owner -ne "$Domain\$DomJoinAccount") {
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUTxzRap+Q7qIOFZKhuxcfGZxD
# 9qCggg9rMIIHsTCCBZmgAwIBAgITNQAAACFv8bfVN7E2EQAAAAAAITANBgkqhkiG
...
# IwYJKoZIhvcNAQkEMRYEFP49M3la6N2Yuru0KjH/mWIZ7qKwMA0GCSqGSIb3DQEB
# AQUABIIBAF4xAMzakT0frMqci1pjsHj+/6uvirF1asUVSDdwGc1ezX7duNH4+8X5
# ex2/WwxYLMpwd1y7gbEeQLUTc1FY+5H4vVxxf3X9YczbHECixQk8IYORKk47Asfn
# BocxVbC7f9Mn3xvjuy1rFsWI2bnvQqcqrKcmdpp/e+XZqeZY+i4rePgUkkazB1MK
# K7UAM4R2fF53EOLjnObKSHLUmGha2Wsc5/nybiMSYrxTyhgjJtMZX63sOZVK521k
# J+D4D3cS6DMtkDD3WjVHdAnbEoVBDd2g9P3zmom2IU3moGiwU7t7vhTwT62/avvC
# 9CGiv7VOxRS/9E4N9jFCNMRVvx8Np8Y=
